...
def build_image_index(img_dir: Union[str, Path]) -> ImageIndex:
    ...
    if not frames:
        raise RuntimeError(f"No images found in {img_dir} with extensions={sorted(_IMG_EXTS)}")
...
def frame_signature(records: Sequence[MotRecord]) -> Tuple[Tuple[int, int, int, int, int], ...]:
    """
    ...
    sig = []
    for r in records:
        sig.append((r.track_id, int(round(r.x)), int(round(r.y)), int(round(r.w)), int(round(r.h))))
    sig.sort()
    return tuple(sig)
...
def infer_frame_offset_from_full_gt(
    ...
    *,
    ...
    Assumes the split GT is a contiguous subsequence of full GT
    ...
    split_recs = read_mot_file(split_gt_path, is_gt=True, drop_ignored=True, strict=False, sort=True)
    full_recs = read_mot_file(full_gt_path, is_gt=True, drop_ignored=True, strict=False, sort=True)
    ...
    # Offsets are frames where the split first-frame signature matches
    sf0 = split_frames_k[0]
    sig0 = split_sigs[sf0]
    matching_full_frames = full_sig_to_frames.get(sig0, [])

    for ff0 in matching_full_frames:
        offset = ff0 - sf0
        ok = True
